...
//if (ForumUser::$forum_user['is_guest'] && isset($_SERVER['PHP_AUTH_USER']))
//	ForumUser::authenticate_user($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW']);

if (ForumUser::$forum_user['g_read_board'] == '0')
{
	http_authenticate_user();
	exit(ForumCore::$lang['No view']);
}
...
function http_authenticate_user()
{
	...
	header('WWW-Authenticate: Basic realm="'.ForumCore::$forum_config['o_board_title'].' External Syndication"');
...
function output_rss($feed)
{
	...
	if (ForumCore::$forum_config['o_show_version'] == '1')
		echo "\t\t".'<generator>HiveBB '.ForumCore::$forum_config['o_cur_version'].'</generator>'."\n";
	else
		echo "\t\t".'<generator>HiveBB</generator>'."\n";
...
function output_atom($feed)
{
	...
	if (ForumCore::$forum_config['o_show_version'] == '1')
		echo "\t".'<generator version="'.ForumCore::$forum_config['o_cur_version'].'">HiveBB</generator>'."\n";
	else
		echo "\t".'<generator>HiveBB</generator>'."\n";
...
if ($action == 'feed')
{
	...
	if (isset($_GET['tid']))
	{
		...
		{
			http_authenticate_user();
			exit(ForumCore::$lang['Bad request']);
		}
		...
		// Setup the feed
		$feed = array(
			'title'		=>	ForumCore::$forum_config['o_board_title'].ForumCore::$lang['Title separator'].$cur_topic['subject'],
			'link'			=>	forum_link(ForumCore::$forum_url['topic'], array($tid, sef_friendly($cur_topic['subject']))),
			'description'	=>	sprintf(ForumCore::$lang['RSS description topic'], $cur_topic['subject']),
			'items'			=>	array(),
			'type'			=>	'posts'
		);
		...
		while ($cur_post = $forum_db->fetch_assoc($result))
		{
			...
			$item = array(
				'id'			=>	$cur_post['id'],
				'title'			=>	$cur_topic['first_post_id'] == $cur_post['id'] ? $cur_topic['subject'] : ForumCore::$lang['RSS reply'].$cur_topic['subject'],
				'link'			=>	forum_link(ForumCore::$forum_url['post'], $cur_post['id']),
			...
			{
				if ($cur_post['email_setting'] == '0' && !ForumUser::$forum_user['is_guest'])
					$item['author']['email'] = $cur_post['email'];

				$item['author']['uri'] = forum_link(ForumCore::$forum_url['user'], $cur_post['poster_id']);
			}
			else if ($cur_post['poster_email'] != '' && !ForumUser::$forum_user['is_guest'])
	...
	else
	{
		...
		if (isset($_GET['fid']) && is_scalar($_GET['fid']) && $_GET['fid'] != '')
		{
			...
			if (count($fids) == 1)
			{
				...
				if (!is_null($forum_name_in_db) && $forum_name_in_db !== false)
					$forum_name = ForumCore::$lang['Title separator'].$forum_name_in_db;
			}
		}
		...
		// Setup the feed
		$feed = array(
			'title'			=>	ForumCore::$forum_config['o_board_title'].$forum_name,
			'link'			=>	forum_link(ForumCore::$forum_url['index']),
			'description'	=>	sprintf(ForumCore::$lang['RSS description'], ForumCore::$forum_config['o_board_title']),
			'items'			=>	array(),
			'type'			=>	'topics'
		);
		...
		while ($cur_topic = $forum_db->fetch_assoc($result))
		{
			...
			$item = array(
				...
				'link'			=>	forum_link(ForumCore::$forum_url['topic_new_posts'], array($cur_topic['id'], sef_friendly($cur_topic['subject']))),
			...
			{
				if ($cur_topic['email_setting'] == '0' && !ForumUser::$forum_user['is_guest'])
					$item['author']['email'] = $cur_topic['email'];

				$item['author']['uri'] = forum_link(ForumCore::$forum_url['user'], $cur_topic['poster_id']);
			}
			else if ($cur_topic['poster_email'] != '' && !ForumUser::$forum_user['is_guest'])
...
else if ($action == 'online' || $action == 'online_full')
{
	...
	require FORUM_ROOT.'lang/'.ForumCore::$forum_config['o_default_lang'].'/index.php';
...
else if ($action == 'stats')
{
	...
	echo sprintf($lang_index['Newest user'], '<a href="'.forum_link(ForumCore::$forum_url['user'], $stats['last_user']['id']).'">'.forum_htmlencode($stats['last_user']['username']).'</a>').'<br />'."\n";
