 ...
 *
 * @copyright (C) 2023 HiveBB, partially based on code (C) 2008-2009 FluxBB.org
...
function preparse_bbcode($text, &$errors, $is_signature = false)
{
	...
	if ($is_signature)
	{
		if (preg_match('#\[quote(=(&quot;|"|\'|)(.*)\\1)?\]|\[/quote\]|\[code\]|\[/code\]|\[list(=([1a\*]))?\]|\[/list\]#i', $text))
			$errors[] = ForumCore::$lang['Signature quote/code/list'];
	}

	if (ForumCore::$forum_config['p_sig_bbcode'] == '1' && $is_signature || ForumCore::$forum_config['p_message_bbcode'] == '1' && !$is_signature)
	{
		...
		if (ForumCore::$forum_config['o_make_links'] == '1')
		{
...
function preparse_tags($text, &$errors, $is_signature = false)
{
	...
	$tags_nested = array('quote' => ForumCore::$forum_config['o_quote_depth'], 'list' => 5, '*' => 5);
	...
	foreach ($split_text as $current)
	{
		...
		if ($equalpos = strpos($current,'='))
		{
			...
			{
				// Empty tag argument
				$errors[] = sprintf(ForumCore::$lang['BBCode error 6'], $current_tag);
				return false;
			}
		...
		{
			$errors[] = sprintf(ForumCore::$lang['BBCode error 3'], $current_tag, $open_tags[$opened_tag]);
			return false;
		}
		...
		if (substr($current, 1, 1) == '/')
		{
			...
			if ($opened_tag == 0 || !in_array($current_tag, $open_tags))
			{
				...
				{
					$errors[] = sprintf(ForumCore::$lang['BBCode error 1'], $current_tag);
					return false;
				}
			}
			else
						...
						if (in_array($current_tag, $open_tags))
						{
							...
							while (!empty($open_tags))
							{
								...
								if (!in_array($temp_tag, $tags_fix))
								{
									...
									$errors[] = sprintf(ForumCore::$lang['BBCode error 5'], $temp_tag);
									return false;
								}
								array_push($temp_opened, $temp_tag);
								array_push($temp_opened_arg, $temp_arg);
		...
		else
		{
			...
			if (in_array($current_tag, $open_tags) && !in_array($current_tag, array_keys($tags_nested)))
			{
				...
				$errors[] = sprintf(ForumCore::$lang['BBCode error 4'], $current_tag);
				return false;
			}
	...
	foreach ($tags_closed as $check)
	{
		if (in_array($check, $open_tags))
		{
			...
			$errors[] = sprintf(ForumCore::$lang['BBCode error 5'], $check);
			return false;
		}
	}
	...
	if ($current_ignore)
	{
		...
		$errors[] = sprintf(ForumCore::$lang['BBCode error 5'], $current_ignore);
		return false;
	}
...
function split_text($text, $start, $end, &$errors, $retab = true)
{
	...
	for ($i = 1; $i < $num_tokens; ++$i)
	{
		...
		if (count($temp) != 2)
		{
			$errors[] = ForumCore::$lang['BBCode code problem'];
	...
	if (ForumCore::$forum_config['o_indent_num_spaces'] != 8 && $retab)
	{
		$spaces = str_repeat(' ', ForumCore::$forum_config['o_indent_num_spaces']);
...
function handle_img_tag($url, $is_signature = false, $alt = null)
{
	...
	$img_tag = '<a href="'.$url.'">&lt;'.ForumCore::$lang['Image link'].'&gt;</a>';

	if ($is_signature && ForumUser::$forum_user['show_img_sig'] != '0')
		$img_tag = '<img class="sigimage" src="'.$url.'" alt="'.forum_htmlencode($alt).'" />';
	else if (!$is_signature && ForumUser::$forum_user['show_img'] != '0')
...
function do_bbcode($text, $is_signature = false)
{
	...
	if (strpos($text, '[quote') !== false)
	{
		...
		$text = preg_replace_callback(
			...
			function ($matches) use ($lang) {
				return '</p><div class="quotebox"><cite>'.str_replace(array('[', '\\"'), array('&#91;', '"'), $matches[2]).' '.ForumCore::$lang['wrote'].':</cite><blockquote><p>';
			},
			$text
		);
	...
	if (($is_signature && ForumCore::$forum_config['p_sig_img_tag'] == '1') || (!$is_signature && ForumCore::$forum_config['p_message_img_tag'] == '1'))
	{
...
function do_smilies($text)
{
	...
	foreach ($smilies as $smiley_text => $smiley_img)
	{
		if (strpos($text, $smiley_text) !== false)
			$text = preg_replace("#(?<=[>\s])".preg_quote($smiley_text, '#')."(?=\W)#m", '<img src="'.FORUM_BASE_URL.'/img/smilies/'.$smiley_img.'" width="15" height="15" alt="'.substr($smiley_img, 0, strrpos($smiley_img, '.')).'" />', $text);
	}
...
function parse_message($text, $hide_smilies)
{
	...
	if (ForumCore::$forum_config['p_message_bbcode'] == '1' && strpos($text, '[') !== false && strpos($text, ']') !== false)
		$text = do_bbcode($text);

	if (ForumCore::$forum_config['o_smilies'] == '1' && ForumUser::$forum_user['show_smilies'] == '1' && $hide_smilies == '0')
...
function parse_signature($text)
{
	...
	if (ForumCore::$forum_config['p_sig_bbcode'] == '1' && strpos($text, '[') !== false && strpos($text, ']') !== false)
		$text = do_bbcode($text, true);

	if (ForumCore::$forum_config['o_smilies_sig'] == '1' && ForumUser::$forum_user['show_smilies'] == '1')
		$text = do_smilies($text);
