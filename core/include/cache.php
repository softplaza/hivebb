...
function generate_quickjump_cache($group_id = false)
{
	...
	foreach ($groups as $group_id)
	{
		...
		$output .= '<form id="qjump" method="get" accept-charset="utf-8" action="'.ForumCore::$base_url.'/viewforum.php">'."\n\t".'<div class="frm-fld frm-select">'."\n\t\t".'<label for="qjump-select"><span><?php echo ForumCore::$lang[\'Jump to\'] ?>'.'</span></label><br />'."\n\t\t".'<span class="frm-input"><select id="qjump-select" name="id">'."\n";
		...
		$output .= "\t\t\t".'</optgroup>'."\n\t\t".'</select>'."\n\t\t".'<input type="submit" id="qjump-submit" value="<?php echo ForumCore::$lang[\'Go\'] ?>" /></span>'."\n\t".'</div>'."\n".'</form>'."\n";
		$output_js = "\n".'(function () {'."\n\t".'var forum_quickjump_url = "'.forum_link(ForumCore::$forum_url['forum']).'";'."\n\t".'var sef_friendly_url_array = new Array('.count($forums).');';
...
function gen_apps()
{
	...
	while ($row = $forum_db->fetch_assoc($result))
		$output[$row['id']] = [
			...
			'url'			=> FORUM_BASE_URL.'/apps/'.$row['id'].'/',
			'path'			=> FORUM_ROOT.'apps/'.$row['id'].'/'
		];

	// Output hooks as PHP code
	if (!write_cache_file(FORUM_CACHE_DIR.'apps_info.php', '<?php'."\n\n".'define(\'FORUM_HOOKS_LOADED\', 1);'."\n\n".'$apps_info = '.var_export($output, true).';'."\n\n".'?>'))
	{
...
function generate_updates_cache()
{
	...
	$result = get_remote_file('http://punbb.informer.com/update/?type=xml&version='.urlencode(ForumCore::$forum_config['o_cur_version']).'&hotfixes='.implode(',', $hotfixes), 8);
