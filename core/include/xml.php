...
function validate_manifest($xml_array, $folder_name)
{
	...
	if (!isset($xml_array['extension']) || !is_array($xml_array['extension']))
		$errors[] = ForumCore::$lang['extension root error'];
	else
	{
		$ext = $xml_array['extension'];
		if (!isset($ext['attributes']['engine']))
			$errors[] = ForumCore::$lang['extension/engine error'];
		else if ($ext['attributes']['engine'] != '1.0')
			$errors[] = ForumCore::$lang['extension/engine error2'];

		if (!isset($ext['id']) || $ext['id'] == '')
			$errors[] = ForumCore::$lang['extension/id error'];
		else if ($ext['id'] != $folder_name)
			$errors[] = ForumCore::$lang['extension/id error2'];

		if (!isset($ext['title']) || $ext['title'] == '')
			$errors[] = ForumCore::$lang['extension/title error'];
		if (!isset($ext['version']) || $ext['version'] == '' || preg_match('/[^a-z0-9\- \.]+/i', $ext['version']))
			$errors[] = ForumCore::$lang['extension/version error'];
		if (!isset($ext['description']) || $ext['description'] == '')
			$errors[] = ForumCore::$lang['extension/description error'];
		if (!isset($ext['author']) || $ext['author'] == '')
			$errors[] = ForumCore::$lang['extension/author error'];
		if (!isset($ext['minversion']) || $ext['minversion'] == '')
			$errors[] = ForumCore::$lang['extension/minversion error'];
		if (isset($ext['minversion']) && version_compare(clean_version(ForumCore::$forum_config['o_cur_version']), clean_version($ext['minversion']), '<'))
			$errors[] = sprintf(ForumCore::$lang['extension/minversion error2'], $ext['minversion']);
		if (!isset($ext['maxtestedon']) || $ext['maxtestedon'] == '')
			$errors[] = ForumCore::$lang['extension/maxtestedon error'];
		...
		if (isset($ext['note']))
		{
			foreach ($ext['note'] as $note)
			{
				if (!isset($note['content']) || $note['content'] == '')
					$errors[] = ForumCore::$lang['extension/note error'];
				if (!isset($note['attributes']['type']) || $note['attributes']['type'] == '')
					$errors[] = ForumCore::$lang['extension/note error2'];
			}
		}
		...
		if (isset($ext['hooks']) && is_array($ext['hooks']))
		{
			if (!isset($ext['hooks']['hook']) || !is_array($ext['hooks']['hook']))
				$errors[] = ForumCore::$lang['extension/hooks/hook error'];
			else
			{
				foreach ($ext['hooks']['hook'] as $hook)
				{
					...
					if (!isset($hook['attributes']['id']) || $hook['attributes']['id'] == '')
						$errors[] = ForumCore::$lang['extension/hooks/hook error2'];
					if (isset($hook['attributes']['priority']) && (!ctype_digit($hook['attributes']['priority']) || $hook['attributes']['priority'] < 0 || $hook['attributes']['priority'] > 10))
						$errors[] = ForumCore::$lang['extension/hooks/hook error3'];
					...
					if (is_array($last_element) && $last_element[0] == T_INLINE_HTML)
						$errors[] = ForumCore::$lang['extension/hooks/hook error4'];
				}
			}
		}
		...
		if (isset($ext['minphpversion'])) {
			if (version_compare(PHP_VERSION, $ext['minphpversion'], "<")) {
				$errors[] = ForumCore::$lang['The minimum required version of PHP'] . $ext['minphpversion'];
			}
		}
		if (isset($ext['maxphpversion'])) {
			if (version_compare(PHP_VERSION, $ext['maxphpversion'], ">")) {
				$errors[] = ForumCore::$lang['The maximum required version of PHP'] . $ext['maxphpversion'];
			}
		}
			...
			foreach (explode(",", $ext['phpextensions']) as $v) {
				...
				if ($v != "") {
					if (!isset($installed_phpextensions[$v])) {
						$errors[] = ForumCore::$lang['PHP extension is required'] . $v;
					}
				}
			}
		}
